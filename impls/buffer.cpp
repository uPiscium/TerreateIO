...
Byte ReadBuffer::Fetch() const {
  if (mCursor >= mBuffer + mSize) {
    return EOB;
  } else {
    return *mCursor;
  }
...
Str ReadBuffer::Fetch(Size const &size) const {
  Str result;
...
Str ReadBuffer::ReadAll() {
  Str result = Str(mCursor, mBuffer + mSize);
  mCursor = mBuffer + mSize;
  return result;
}
